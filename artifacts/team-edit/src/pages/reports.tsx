...
interface ReportTask {
  ...
  client: string | null; color: string;
  dueDate: string | null; createdAt: string; updatedAt: string;
  assignee: Person | null; coordinator: Person | null;
}

const STATUS_ORDER = ["pending", "in_progress", "captacao", "in_revision", "review", "completed", "paused", "cancelled"];
...
const STATUS_COLORS: Record<string, { bg: string; color: string }> = {
  ...
  captacao:    { bg: "#ede9fe", color: "#7c3aed" },
  in_revision: { bg: "#ffedd5", color: "#9a3412" },
...
export default function Reports() {
  ...
  const hasFilters = fStatus !== "all" || fClient !== "all" || fEditor !== "all"
    || fCoord !== defaultCoord || !!search;
  const clearFilters = () => {
    setFStatus("all"); setFClient("all"); setFEditor("all");
    setFCoord(defaultCoord); setSearch("");
  };
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  col.c-code { width:72px; } col.c-title { width:240px; } col.c-client { width:110px; }
  col.c-editor { width:130px; } col.c-coord { width:130px; } col.c-status { width:104px; }
  col.c-due { width:72px; } col.c-created { width:72px; }
...
<div class="summary">
  ...
  <div class="stat"><div class="lbl">Em edição</div><div class="val" style="color:#1d4ed8">${inProg}</div></div>
  <div class="stat"><div class="lbl">Aprovadas</div><div class="val" style="color:#16a34a">${completed}</div></div>
</div>
<table>
  <colgroup>
    <col class="c-code"><col class="c-title"><col class="c-client">
    <col class="c-editor"><col class="c-coord"><col class="c-status">
    <col class="c-due"><col class="c-created">
  </colgroup>
  <thead>
    <tr>
      <th>#</th><th>Tarefa</th><th>Cliente</th><th>Editor</th>
      <th>Atendimento</th><th>Status</th><th>Entrega</th><th>Criada</th>
    </tr>
  </thead>
  <tbody>${rows}</tbody>
</table>
  ...
  // ── Exportar CSV ─────────────────────────────────────────────────────────
  const exportCSV = () => {
    const headers = ["#","Tarefa","Cliente","Editor","Atendimento","Status","Entrega","Criada em"];
    const rows = filtered.map(t => [
      t.taskCode ?? "",
      t.title,
      t.client ?? "",
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          { label: "Em edição",        value: inProg,    cls: "text-blue-600" },
          { label: "Aprovadas",        value: completed, cls: "text-green-600" },
        ].map(k => (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-[hsl(var(--muted))]/10">
                  {["#","Tarefa","Cliente","Editor","Atendimento","Status","Entrega","Criada"].map(h => (
