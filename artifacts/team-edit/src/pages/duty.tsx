        ...
        <div>
          ...
          <div className="flex flex-col gap-2">
            {Array.from({ length: numWeeks }).map((_, weekIdx) => (
              <div key={weekIdx} className="grid grid-cols-7 gap-1 p-2 rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))]">
                {Array.from({ length: 7 }).map((_, dayIdx) => {
                  const cellIdx = weekIdx * 7 + dayIdx;
                  const day     = cellIdx - firstDayOffset + 1;

                  if (day < 1 || day > daysInMonth) {
                    return <div key={dayIdx} className="min-h-[90px] rounded-xl" />;
                  }
                  ...
                  return (
                    <div
                      key={iso}
                      className={`rounded-xl flex flex-col p-1.5 min-h-[90px] transition-colors ${
                        isCurWknd
