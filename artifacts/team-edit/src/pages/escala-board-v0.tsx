...
function scoreInfo(hoursToday: number, dailyCap: number) {
  ...
  if (hoursToday === 0)     return { label: "disponível",    color: "#6366f1" };
  if (hoursToday < cap / 2) return { label: "ocupado",       color: "#facc15"             };
  if (hoursToday < cap)     return { label: "muito ocupado", color: "#fb923c"             };
  return                           { label: "no limite",     color: "#f87171"             };
}
...
export default function EscalaBoardV0() {
  usePageTitle("Agenda");
  ...
  return (
    <div ref={pageRef} className="min-h-screen px-6 py-10 max-w-5xl mx-auto">

      <div className="mb-10">
        <h1 className="text-3xl font-black tracking-tighter leading-none select-none"
          style={{ letterSpacing: "-0.02em" }}>agenda</h1>
        <p className="text-sm mt-2 font-medium tracking-widest uppercase"
          style={{ color: "hsl(var(--muted-foreground))" }}>quem tá na linha</p>
      </div>
      ...
      <div className="relative max-w-sm mb-10">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="buscar editor…"
          className="w-full h-11 pl-11 pr-4 rounded-full text-sm font-medium focus:outline-none"
          style={{ background: "hsl(var(--muted))", border: "1px solid hsl(var(--border))", color: "hsl(var(--foreground))" }} />
      </div>

      {loading ? (
        <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))" }}>
          {[...Array(6)].map((_, i) => <div key={i} className="rounded-2xl animate-pulse h-48" style={{ background: "hsl(var(--muted))" }} />)}
        </div>
      ) : (
        <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))" }}>
          {filtered.map(editor => {
            ...
            return (
              <button key={editor.id} onClick={() => navigate(`/agenda/${editor.id}`)}
                className="group relative rounded-2xl overflow-hidden cursor-pointer flex flex-col transition-all duration-200"
                style={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", outline: "none" }}
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.boxShadow=`0 0 0 0.5px ${color}, 0 8px 40px ${color}28`; }}
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.boxShadow=""; }}>

                {/* Corpo */}
                <div className="flex flex-col items-center px-5 pt-6 pb-5 gap-3 flex-1">

                  {/* Avatar */}
                  <div style={{ width: 80, height: 80 }}>
                    <div className="w-full h-full rounded-full overflow-hidden">
                      <Avatar name={editor.name} avatarUrl={editor.avatarUrl} size={80} />
                    </div>
                  </div>

                  {/* Nome + login */}
                  <div className="text-center w-full">
                    <p className="text-base font-black leading-tight truncate">{editor.name.split(" ")[0]}</p>
                    <p className="text-[10px] font-mono mt-0.5 truncate" style={{ color: "hsl(var(--muted-foreground)/0.5)" }}>
                      {editor.login}
                    </p>
                  </div>
                  ...
                  <div className="flex items-center justify-between w-full">
                    ...
                    <span className="text-[10px] font-mono" style={{ color: "hsl(var(--muted-foreground)/0.5)" }}>
                      {editor.taskCount === 0 ? "—" : `${editor.taskCount}t`}
                    </span>
                  </div>
          ...
          {!loading && filtered.length === 0 && (
            <p className="col-span-full text-center py-16 text-sm" style={{ color: "hsl(var(--muted-foreground))" }}>nenhum editor encontrado</p>
          )}
        </div>
      )}
