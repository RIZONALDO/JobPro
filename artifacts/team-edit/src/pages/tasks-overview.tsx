...
export default function TasksOverview() {
  ...
  const dateMeta = (dateKey: string | null | undefined) => {
    ...
    if (!k) return { dayNum: "–", dayName: "s/d", mon: "", isToday: false, isPast: false };
    const d = new Date(k + "T12:00:00");
    return { dayNum: String(d.getDate()), dayName: DAY_PT[d.getDay()], mon: MON_PT[d.getMonth()], isToday: k === TODAY, isPast: k < TODAY };
  };
                  ...
                  {sortedFiltered.map((t, rowIndex) => {
                    ...
                    return (
                      <Fragment key={t.id}>
                        ...
                        {isExpanded && (
                          isLoadingSubs ? (
                            <tr><td colSpan={columns.length + 1} className="px-8 py-3 text-xs text-[hsl(var(--muted-foreground))]">Carregando…</td></tr>
                          ) : subList.map(s => (
