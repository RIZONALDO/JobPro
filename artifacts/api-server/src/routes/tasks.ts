...
router.put("/tasks/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  if (role === "editor") {
    ...
    if (status) {
      const s = String(status);
      // Coordinator controls: completed, paused, cancelled — allowed from any status
      const allowed = ["completed","paused","cancelled","pending","in_progress","in_revision","review","captacao"];
      if (allowed.includes(s)) {
        update.status = s;
        eventComment = revisionComment ? String(revisionComment).trim() : "";
      } else {
        res.status(400).json({ error: "Transição de status inválida" }); return;
      }
    }
  }
